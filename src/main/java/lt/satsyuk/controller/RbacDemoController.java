...
public class RbacDemoController {
    ...
    public AppResponse<AccessProbeResponse> admin(Authentication authentication) {
    ...
    public AppResponse<AccessProbeResponse> customersRead(Authentication authentication) {
    ...
    public AppResponse<AccessProbeResponse> customersUpdate(Authentication authentication) {
    ...
    public AppResponse<AccessProbeResponse> customersDelete(Authentication authentication) {
    ...
    public AppResponse<AccessProbeResponse> reportsRead(Authentication authentication) {
    ...
    public AppResponse<AccessProbeResponse> reportsCreate(Authentication authentication) {
        return probe(authentication, "reports:create");
    }

    private AppResponse<AccessProbeResponse> probe(Authentication authentication, String granted) {
        List<String> authorities = authentication.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .sorted()
                .toList();
        return AppResponse.ok(new AccessProbeResponse(authentication.getName(), granted, authorities));
    }
}
